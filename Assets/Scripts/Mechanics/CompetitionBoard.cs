...
public class CompetitionBoard : MonoBehaviour
{
	...
	new int[] flyerWeek = new int[flyerCount];
	...
	public void SetFlyers ()
	{
		...
		for (int i = 0; i < flyerType.Length; i++) {
			...
			flyerWeek [i] = dayCycle.Week;

			if (flyerType [i] == 2)
				flyerWeek [i] += 3; //add a few weeks for the major competitions
			...
			flyerScript.eventWeek = flyerWeek[i];
			...
			flyerScript.flyerDateText = "Week " + flyerWeek[i] + ", " + dayOfWeek + " " + flyerHour [i] + ":00";
			flyerScript.flyerDetailsText = flyerDetails [Random.Range (0, flyerDetails.Length)];

			//create the major event progression data if it doesnt exist
			if (!PlayerPrefs.HasKey ("MajorEventData")) {
				SaveLoad.SaveInt ("MajorEventData", 1);
			}
			
			//if its a major competition
			if (flyerType [i] == 2) {
				flyerScript.isMajor = true;
				flyerScript.majorIndex = SaveLoad.LoadInt ("MajorEventData");
				tmpClass = SaveLoad.LoadInt ("MajorEventData");
				flyerScript.firstPrize += 100;
				flyerScript.secondPrize += 75;
				flyerScript.thirdPrize += 50;
			}
			...
			if (tmpClass == 3)
				flyerScript.eventClass = "Changeme1 Tire";
			if (tmpClass == 4)
				flyerScript.eventClass = "Changeme2 Tire";

			flyerScript.flyerTitle = flyerScript.eventClass + " Event";

			//set the map
			if (flyerType [i] == 2) {
				if (flyerScript.majorIndex == 1)
					flyerScript.eventMap = "Competition 2";
				if (flyerScript.majorIndex == 2)
					flyerScript.eventMap = "Race 2";
				if (flyerScript.majorIndex == 3)
					flyerScript.eventMap = "Competition 3";
				if (flyerScript.majorIndex == 4)
					flyerScript.eventMap = "Race 3";
			} else {
				if (tmpMap == 1)
			...
			if (flyerType [i] == 2) {
				if (tmpClass == 1)
					flyerScript.difficulty = 35;
				if (tmpClass == 2)
					flyerScript.difficulty = 40;
				flyerScript.difficultyLevel = 3; //most difficult
			} else {
